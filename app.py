...
from layouts.general_layouts import rename_file,upload_component
...
# función para obtener il div del grafo de la red
...
app.layout = html.Div([
    ...
    # Dcc.store para el json
    dcc.Store(id='network-elements'),
    # Dcc.store para los elementos del grafo
    dcc.Store(id='network-graph-elements', data=[]),
    dcc.Store(id='image-data-store'),  # Agregar este componente a tu layout
    navbar,
    html.Div(id='page-content',children=cytoscape_component),
])
...
def display_page(form_data,pathname,n_clicks,imageData):
    ...
    # Aquí puedes utilizar los datos del formulario para personalizar el componente Cytoscape
...
# TODO : ACTUALIZAR EL GRAFO CUANDO SE CARGA EL JSON
@app.callback(
